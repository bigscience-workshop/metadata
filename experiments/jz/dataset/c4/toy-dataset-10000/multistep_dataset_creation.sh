...
for filename in $DATASET_FILES_DIR_INIT/*; do
    ...
    if [ $FILENAME_END != "lock" ]; then    
        ...
        --export=ALL,FILENAME=$FILENAME,NEW_FILENAME=$NEW_FILENAME,OUT_DIR=$OUT_DIR,METADATA_TO_INCLUDE=$METADATA_TO_INCLUDE,DATASET_FILES_DIR=$DATASET_FILES_DIR,PATH_OR_URL_FLAIR_NER_MODEL=$PATH_OR_URL_FLAIR_NER_MODEL,MAP_BATCH_SIZE=$MAP_BATCH_SIZE \
        ...
        --export=ALL,FILENAME=$NEW_FILENAME,NEW_FILENAME=$NEW_FILENAME,OUT_DIR=$OUT_DIR,METADATA_TO_INCLUDE=$METADATA_TO_INCLUDE,DATASET_FILES_DIR=$DATASET_FILES_DIR,PATH_OR_URL_FLAIR_NER_MODEL=$PATH_OR_URL_FLAIR_NER_MODEL,MAP_BATCH_SIZE=$MAP_BATCH_SIZE \
        01_add_metadata_to_toy_c4_dataset.slurm | cut -d " " -f 4)
