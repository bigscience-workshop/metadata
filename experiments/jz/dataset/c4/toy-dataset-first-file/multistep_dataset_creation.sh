DATASET_FILES_DIR_INIT=$SCRATCH/new_dataset/c4-en-reduced-first-file

PROCESS_DIR=$SCRATCH/new_dataset/c4-en-reduced-in-progess-first-file
...
for filename in $DATASET_FILES_DIR_INIT/*; do
    ...
    if [ $FILENAME_END != "lock" ]; then    
        ...
        ID_JOB3=$(sbatch --dependency=afterok:$ID_JOB2 --job-name=modelling-metadata-c4-dataset-toy-add-metadata-entity \
        --qos=qos_gpu-t3 \
        --account=six@gpu \
        --gres=gpu:1 \
        --constraint=v100-16g \
